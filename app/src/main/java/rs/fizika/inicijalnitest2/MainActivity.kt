...
class MainActivity:AppCompatActivity(){
    ...
    private fun showHome(){
        ...
        hero.addView(center("FIZIKA 2",38f,Color.WHITE).apply{setTypeface(null,Typeface.BOLD);setPadding(0,dp(12),0,0)})
        hero.addView(center("PREMIUM • Inicijalni test",16f,Color.parseColor("#DDEEFF")).apply{setTypeface(null,Typeface.BOLD);setPadding(0,dp(4),0,0)})
        hero.addView(center("v = v₀ + at   •   F = ma   •   E = Ek + Ep",17f,Color.parseColor("#9ED7FF")).apply{setPadding(0,dp(20),0,0)})
        hero.addView(center("MEHANIKA  •  KRETANJE  •  SILA  •  ENERGIJA",12f,Color.parseColor("#D3A94B")).apply{letterSpacing=.05f;setTypeface(null,Typeface.BOLD);setPadding(0,dp(10),0,0)})
        hero.addView(center("„Ako želiš da pronađeš tajne svemira, misli u terminima energije, frekvencije i vibracije.“\nNikola Tesla",14f,Color.WHITE).apply{setTypeface(Typeface.create(Typeface.SERIF,Typeface.ITALIC));setPadding(dp(8),dp(20),dp(8),0)})
    ...
    private fun showAreas()=showInfo("Gradivo","UVOD I MERENJA\n• SI jedinice, skalari i vektori\n\nKINEMATIKA\n• put, pomeraj, brzina i ubrzanje\n• grafici kretanja, slobodan pad i kružno kretanje\n\nDINAMIKA\n• Njutnovi zakoni, trenje i gravitacija\n\nRAD, ENERGIJA I SNAGA\n• mehanički rad, kinetička i potencijalna energija\n• zakon održanja energije i snaga\n\nIMPULS I ROTACIJA\n• impuls, zakon održanja impulsa i moment sile")
    ...
    private fun inferArea(q:String):String{val s=q.lowercase();return when{listOf("vektor","si jedin","brzina","ubrz","put ","pomeraj","grafik","period","frekvenc","pad","kruž").any{s.contains(it)}->"Kinematika i merenja";listOf("njutn","sila","tren","gravit","težin","masa").any{s.contains(it)}->"Dinamika";listOf("rad ","energ","snag").any{s.contains(it)}->"Rad i energija";listOf("impuls","moment","rotac").any{s.contains(it)}->"Impuls i rotacija";else->"Ostalo gradivo"}}
